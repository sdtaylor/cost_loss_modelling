...
for(loss_cost in possible_loss_costs){
  ...
  forecasts_binary = raster::calc(forecasts, function(x, threshold){(x>=a)*1})
  
  expense_perfect = (sum(as.vector(observations)) * treatment_cost) / total_cells
  expense_never = (sum(as.vector(observations)) * loss_cost) / total_cells
  expense_maximimum = min(treatment_cost, expense_never)
  
  forecast_treatment_cost = (sum(as.vector(forecasts_binary)) * treatment_cost)
  forecast_fn_cost = false_negative_costs(forecasts_binary, observations, loss_cost)
