...
download_dir = '../../raw_data/idaho_fire/downloaded_modis/'
...
####################################################################################
# Next convert them to more friendly tif format using the MODIS Reprojection tool
# https://lpdaac.usgs.gov/tools/modis_reprojection_tool
mrtPath='/home/shawn/bin/bin/resample'
resample_config_file='./resample_config.prm'

hdfFiles = glob.glob(download_dir+'/*hdf')

for this_file in hdfFiles:
    #Keeps the 1st 3 sections of the modis filename. Like MOD13A1-A2008144-h10v04
    output_file=download_dir+'-'.join(os.path.basename(this_file).split('.')[0:3])+'.tif'
    call([mrtPath, '-i', this_file, '-o', output_file, '-p', resample_config_file ])
